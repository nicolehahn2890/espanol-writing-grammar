...
- `apple-touch-icon.png` (180×180, identical to `assets/icon-180.png`): true pixel-art — a white hand-drawn "Ñ" (pixel-grid letterform + bold tilde) on a **plain solid lavender (`--purple` #b58cf0)** background with white "+" sparkles. **No border/frame** and no gradient (the gradient's bands read as ugly stripes, and a grape frame looked boxed-in at home-screen size). Corners are square — iOS applies its own rounded mask.
- **Crisp, not blurry:** the icon is authored on a small **36×36** logical grid and scaled ×5 with nearest-neighbour to 180×180, so every pixel is a hard block. **Do not** anti-alias or export from a smooth/vector source — it must stay blocky.
