...
- `apple-touch-icon.png` (180×180, identical to `assets/icon-180.png`): true pixel-art — a white hand-drawn "Ñ" (pixel-grid letterform + bold tilde) on a **solid lavender (`--purple` #b58cf0)** background, with a 2-cell grape pixel border and white "+" sparkles. Corners are square — iOS applies its own rounded mask. (A gradient background was dropped — its bands read as ugly stripes at home-screen size.)
